...
# --- Personalized Climate Alerts and Recommendations ---
st.header("Personalized Climate Alerts and Recommendations")
...
def display_alerts():
    ...
    # Temperature Alert
    if temp_avg > 35:
        alerts.append(("🔥 Heatwave Warning", "The average temperature is forecasted to exceed 35°C. It's recommended to stay hydrated, avoid outdoor activities during peak heat, and use sun protection."))
    elif temp_avg > 30:
        alerts.append(("☀️ Warm Weather Advisory", "Temperatures are expected to remain high. Be cautious of prolonged outdoor exposure during peak heat hours."))

    # Rainfall Alert
    if rain_avg > 200:
        alerts.append(("🌧️ Flood Risk Alert", "Heavy rainfall is expected. Prepare for potential flooding in low-lying areas. Keep emergency supplies ready and avoid waterlogged areas."))
    elif rain_avg < 50:
        alerts.append(("💧 Drought Risk Alert", "Low rainfall is forecasted, which may lead to drought conditions. Conserve water and avoid unnecessary usage."))

    # AQI Alert
    if aqi_avg > 150:
        alerts.append(("🌫️ Poor Air Quality", "Air quality is forecasted to be unhealthy. Limit outdoor activities, especially for sensitive groups (children, elderly, respiratory patients)."))
    elif aqi_avg > 100:
        alerts.append(("🚨 Moderate Air Quality Alert", "Air quality may be moderate. Consider reducing outdoor activities if you are sensitive to pollution."))

    # Wind Speed Alert
    if wind_avg < 5:
        alerts.append(("🍃 Low Wind Speed Alert", "Low wind speed may lead to stagnant air and poor pollutant dispersion. Ensure good indoor ventilation to maintain air quality."))
...
fig_temp = px.line(
    x=range(1, 13), y=temp_forecast, 
    labels={'x': 'Month', 'y': 'Temperature (°C)'},
    title="Monthly Temperature Forecast",
    line_shape="spline",
    markers=True
)
...
st.subheader("Rainfall Forecast (mm)")
fig_rain = px.bar(
    x=range(1, 13), y=rain_forecast, 
    labels={'x': 'Month', 'y': 'Rainfall (mm)'},
    title="Monthly Rainfall Forecast",
...
fig_humidity = px.area(
    x=range(1, 13), y=humidity_forecast,
    labels={'x': 'Month', 'y': 'Humidity (%)'},
    title="Monthly Humidity Forecast",
...
fig_wind = px.box(
    y=wind_forecast, 
    labels={'y': 'Wind Speed (km/h)'},
    title="Monthly Wind Speed Distribution"
)
